...
"""
data.py
- This module provides data acquisition and preprocessing utilities for the
  Stalking Stocks project, focused on Series-in/Series-out helpers that the
  Streamlit frontend can call directly.
Manual (pure pandas) implementations for:
    - Missing-value handling (forward-fill/back-fill)
    - Non-trading day filtering (weekend removal)
    - Outlier handling via Tukey IQR (mask or clip)
Notes:
    - Functions operate on a single pandas Series and return a single Series.
    - Designed to be composable (chainable) and fast for interactive use.
"""
...
try:
    ...
    def timer(fn):  # no-op fallback
        return fn

# =====================================================================
# ACTIVE FUNCTIONS (USED IN CURRENT BUILD)
# Series-in → Series-out helpers with consistent docstrings and timing.
# =====================================================================
@timer
...
@timer
def clean_outliers_iqr(series: pd.Series, replace_with_nan: bool = True, k: float = 3.0) -> pd.Series:
    """
